...
- [x] Subtopic pages now use the same two column shape as the landing page. Headlines ran the full width, 131 characters on a single line, well past the 50 to 75 that Nielsen Norman recommends. The story column now sits at the same measure as the landing page river and the newsletter takes the sticky rail beside it, so the space is used rather than left blank. Down to 44 characters a line. The story grid drops to two or three columns to suit the narrower column, and everything stacks on phones.
